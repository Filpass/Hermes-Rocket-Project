...
with open(csv_filename, mode="w", newline="") as csv_file:
    ...
    writer.writerow([
        "Time (s)", "Timestamp", "Pressure (hPa)", "Temperature (°C)", "Altitude (m)",
...
# Start time for calculating elapsed time in seconds
start_time_seconds = time.time()

try:
    while True:
        # Calculate elapsed time (with decimals for seconds)
        elapsed_time = round(time.time() - start_time_seconds, 2)
        ...
        data = f"{elapsed_time},{timestamp},{pressure:.2f},{temperature:.2f},{altitude:.2f}," \
        ...
        with open(csv_filename, mode="a", newline="") as csv_file:
            writer = csv.writer(csv_file)
            writer.writerow([
                elapsed_time, timestamp, pressure, temperature, altitude,
        ...
        # Adjust the time between readings if needed (e.g., to simulate a lower sampling rate)
        time.sleep(0.1)  # 0.1Hz (10 seconds per reading) or adjust this as necessary
